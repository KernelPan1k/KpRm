...
Dim $cu
If $cu Then _zm("[I] prepareRemove " & $at)
If $ct Then
_yx($at)
_yf($at)
EndIf
Local Const $cv = FileGetAttrib($at)
If StringInStr($cv, "R") Then
FileSetAttrib($at, "-R", $cs)
EndIf
If StringInStr($cv, "S") Then
FileSetAttrib($at, "-S", $cs)
EndIf
If StringInStr($cv, "H") Then
FileSetAttrib($at, "-H", $cs)
EndIf
If StringInStr($cv, "A") Then
FileSetAttrib($at, "-A", $cs)
EndIf
EndFunc
Func _10h($cw, $b3, $cx = Null, $ct = False)
Dim $cu
If $cu Then _zm("[I] RemoveFile " & $cw)
Local Const $cy = _zt($cw)
If $cy Then
If $cx And StringRegExp($cw, "(?i)\.(exe|com)$") Then
Local Const $cz = FileGetVersion($cw, "CompanyName")
If @error Then
Return 0
ElseIf Not StringRegExp($cz, $cx) Then
Return 0
EndIf
EndIf
_0zz($b3, 'element', $cw)
_10g($cw, 0, $ct)
Local $d0 = FileDelete($cw)
If $d0 Then
If $cu Then
_zm("  [OK] File " & $cw & " deleted successfully")
EndIf
Return 1
EndIf
Return 2
...
Dim $cu
If $cu Then _zm("[I] RemoveFolder " & $at)
Local $cy = _zu($at)
If $cy Then
_0zz($b3, 'element', $at)
_10g($at, 1, $ct)
Local Const $d0 = DirRemove($at, $q)
If $d0 Then
If $cu Then
_zm("  [OK] Directory " & $at & " deleted successfully")
EndIf
Return 1
EndIf
Return 2
...
Func _10j($at, $cw, $d1)
Dim $cu
If $cu Then _zm("[I] FindGlob " & $at & " " & $cw)
Local Const $d2 = $at & "\" & $cw
Local Const $5z = FileFindFirstFile($d2)
Local $d3 = []
If $5z = -1 Then
Return $d3
EndIf
...
EndIf
Local $78 = "", $79 = "", $5x = "", $7a = ""
Local $bd = _xe($d4, $78, $79, $5x, $7a)
Local $d7 = $5x & $7a
For $d8 = 1 To UBound($d5) - 1
If $d5[$d8][3] And $d6 = $d5[$d8][1] And StringRegExp($d7, $d5[$d8][3]) Then
Local $ba = 0
Local $ct = False
If $d5[$d8][4] = True Then
$ct = True
EndIf
If $d6 = 'file' Then
$ba = _10h($d4, $d5[$d8][0], $d5[$d8][2], $ct)
ElseIf $d6 = 'folder' Then
$ba = _10i($d4, $d5[$d8][0], $ct)
EndIf
EndIf
Next
EndFunc
Func _10l($at, $d5, $d9 = -2)
Dim $cu
If $cu Then _zm("[I] RemoveAllFileFromWithMaxDepth " & $at)
Local $46 = _x2($at, "*.exe;*.txt;*.lnk;*.log;*.reg;*.zip;*.dat;*.scr;*.com", $t, $d9, $y, $10)
If @error <> 0 Then
Return Null
EndIf
For $4g = 1 To $46[0]
...
Func _10m($at, $d5)
Dim $cu
If $cu Then _zm("[I] RemoveAllFileFrom " & $at)
Local Const $d2 = $at & "\*"
Local Const $5z = FileFindFirstFile($d2)
If $5z = -1 Then
Return Null
...
Dim $cu
If $cu Then _zm("[I] RemoveRegistryKey " & $av)
If $ct = True Then
_yx($av)
_yf($av, $8c)
EndIf
...
If $cu Then
If $ba = 1 Then
_zm("  [OK] " & $av & " deleted successfully")
ElseIf $ba = 2 Then
_zm("  [X] " & $av & " deleted failed")
EndIf
EndIf
EndIf
Return $ba
...
Dim $cu
Local $ba = Null
If $cu Then _zm("[I] CloseProcessAndWait " & $9g)
If 0 = ProcessExists($9g) Then Return 0
If $ct = True Then
$ba = _yt($9g)
If 1 = $ba Then
If $cu Then _zm("  [OK] Proccess " & $9g & " stopped successfully")
Return 1
EndIf
Else
ProcessClose($9g)
...
$da -= 1
Sleep(250)
Until($da = 0 Or 0 = ProcessExists($9g))
$ba = ProcessExists($9g)
If 0 = $ba Then
If $cu Then _zm("  [OK] Proccess " & $9g & " stopped successfully")
Return 1
...
Dim $da
Dim $cu
If $cu Then _zm("[I] RemoveAllProcess")
Local $dc = ProcessList()
For $4g = 1 To $dc[0][0]
Local $dd = $dc[$4g][0]
Local $de = $dc[$4g][1]
For $da = 1 To UBound($db) - 1
If StringRegExp($dd, $db[$da][1]) Then
_10o($de, $db[$da][2])
_0zz($db[$da][0], "process", $dd)
EndIf
Next
Next
EndFunc
Func _10q($df)
Dim $cu
If $cu Then _zm("[I] RemoveScheduleTask")
For $4g = 1 To UBound($df) - 1
RunWait('schtasks.exe /delete /tn "' & $df[$4g][1] & '" /f', @TempDir, @SW_HIDE)
Next
EndFunc
Func _10r($df)
Dim $cu
If $cu Then _zm("[I] UninstallNormaly")
Local Const $ay = _zs()
For $4g = 1 To UBound($ay) - 1
For $dg = 1 To UBound($df) - 1
Local $dh = $df[$dg][1]
Local $di = $df[$dg][2]
Local $dj = _10j($ay[$4g], "*", $dh)
For $dk = 1 To UBound($dj) - 1
Local $dl = _10j($dj[$dk], "*", $di)
For $dm = 1 To UBound($dl) - 1
If _zt($dl[$dm]) Then
RunWait($dl[$dm])
_0zz($df[$dg][0], "uninstall", $dl[$dm])
EndIf
Next
Next
Next
...
Func _10s($df)
Dim $cu
If $cu Then _zm("[I] RemoveAllProgramFilesDir")
Local Const $ay = _zs()
For $4g = 1 To UBound($ay) - 1
_10m($ay[$4g], $df)
Next
...
Func _10t($df)
Dim $cu
If $cu Then _zm("[I] RemoveAllSoftwareKeyList")
Local $7x = ""
If @OSArch = "X64" Then $7x = "64"
Local $dn[2] = ["HKCU" & $7x & "\SOFTWARE", "HKLM" & $7x & "\SOFTWARE"]
For $59 = 0 To UBound($dn) - 1
Local $4g = 0
While True
$4g += 1
Local $aw = RegEnumKey($dn[$59], $4g)
If @error <> 0 Then ExitLoop
For $dg = 1 To UBound($df) - 1
If $aw And $df[$dg][1] Then
If StringRegExp($aw, $df[$dg][1]) Then
Local $do = $dn[$59] & "\" & $aw
_10n($do, $df[$dg][0])
EndIf
EndIf
...
Func _10u($df)
Dim $cu
If $cu Then _zm("[I] RemoveUninstallStringWithSearch")
For $4g = 1 To UBound($df) - 1
Local $do = _zq($df[$4g][1], $df[$4g][2], $df[$4g][3])
If $do And $do <> "" Then
_10n($do, $df[$4g][0])
EndIf
Next
...
Func _10v($df)
Dim $cu
If $cu Then _zm("[I] RemoveAllRegistryKeys")
For $4g = 1 To UBound($df) - 1
_10n($df[$4g][1], $df[$4g][0], $df[$4g][2])
Next
EndFunc
Func _10w($df)
For $4g = 1 To UBound($df) - 1
If FileExists($df[$4g][1]) Then
Local $dp = _x1($df[$4g][1])
If @error = 0 Then
For $dk = 1 To $dp[0]
_10h($df[$4g][1] & '\' & $dp[$dk], $df[$4g][0], $df[$4g][2], $df[$4g][3])
Next
EndIf
EndIf
Next
...
Dim $ds
Dim $cg
Local Const $cx = "(?i)^Farbar"
Local Const $dt = "(?i)^FRST.*\.exe$"
Local Const $du = "(?i)^FRST-OlderVersion$"
Local Const $dv = "(?i)^(FRST|fixlist|fixlog|Addition|Shortcut).*\.(exe|txt)$"
Local Const $dw = "(?i)^FRST"
Local Const $dx[1][3] = [[$dq, $dt, False]]
Local Const $dy[1][5] = [[$dq, 'file', $cx, $dv, False]]
Local Const $dz[1][5] = [[$dq, 'folder', Null, $du, False]]
Local Const $e0[1][5] = [[$dq, 'folder', Null, $dw, True]]
_vv($cb, $dx)
_vv($cc, $dy)
_vv($ce, $dy)
_vv($cc, $dz)
...
Dim $ch
Local $dq = "zhp"
Local Const $b0[1][2] = [[$dq, "(?i)^ZHP$"]]
Local Const $e1[1][5] = [[$dq, 'folder', Null, "(?i)^ZHP$", True]]
_vv($ch, $b0)
_vv($cm, $e1)
EndFunc
_10y()
Func _10z()
Local Const $e2 = Null
Local Const $dq = "zhpdiag"
Dim $cb
Dim $cc
Dim $cd
Dim $ce
Dim $cg
Local Const $dt = "(?i)^ZHPDiag.*\.exe$"
Local Const $du = "(?i)^ZHPDiag.*\.(exe|txt|lnk)$"
Local Const $dv = "(?i)^PhysicalDisk[0-9]_MBR\.bin$"
Local Const $dx[1][3] = [[$dq, $dt, False]]
Local Const $dy[1][5] = [[$dq, 'file', $e2, $du, False]]
Local Const $dz[1][5] = [[$dq, 'file', Null, $dv, True]]
_vv($cb, $dx)
_vv($cc, $dy)
_vv($ce, $dy)
_vv($cd, $dy)
...
Func _110()
Local Const $e2 = Null
Local Const $e3 = "zhpfix"
Dim $cb
Dim $cc
Dim $ce
Local Const $dt = "(?i)^ZHPFix.*\.exe$"
Local Const $du = "(?i)^ZHPFix.*\.(exe|txt|lnk)$"
Local Const $dx[1][3] = [[$e3, $dt, False]]
Local Const $dy[1][5] = [[$e3, 'file', $e2, $du, False]]
_vv($cb, $dx)
_vv($cc, $dy)
_vv($ce, $dy)
EndFunc
...
Func _111()
Local Const $e2 = Null
Local Const $e3 = "zhplite"
Dim $cb
Dim $cc
Dim $ce
Local Const $dt = "(?i)^ZHPLite.*\.exe$"
Local Const $du = "(?i)^ZHPLite.*\.(exe|txt|lnk)$"
Local Const $dx[1][3] = [[$e3, $dt, False]]
Local Const $dy[1][5] = [[$e3, 'file', $e2, $du, False]]
_vv($cb, $dx)
_vv($cc, $dy)
...
_111()
Func _112($br = False)
Local Const $cx = "(?i)^Malwarebytes Anti-Rootkit$"
Local Const $e4 = "(?i)^Malwarebytes"
Local Const $dq = "mbar"
Dim $cb
Dim $cc
Dim $ce
Dim $ch
Local Const $dt = "(?i)^mbar.*\.exe$"
Local Const $du = "(?i)^mbar"
Local Const $dx[1][3] = [[$dq, $dt, True]]
Local Const $dy[1][2] = [[$dq, $cx]]
Local Const $dz[1][5] = [[$dq, 'file', $e4, $dt, False]]
Local Const $e0[1][5] = [[$dq, 'folder', $cx, $du, False]]
_vv($cb, $dx)
_vv($cc, $dz)
_vv($ce, $dz)
_vv($cc, $e0)
...
Func _113()
Dim $cr
Local Const $dq = "malwarebytes_tools"
Local Const $dx[1][4] = [[$dq, @AppDataCommonDir & "\Malwarebytes\Malwarebytes' Anti-Malware\Quarantine", Null, True]]
_vv($cr, $dx)
EndFunc
_113()
Func _114()
Local Const $dq = "roguekiller"
Dim $cb
Dim $ci
Dim $cj
Dim $cf
...
Local Const $dt = "(?i)^RogueKiller"
Local Const $du = "(?i)^RogueKiller.*\.(exe|lnk|txt)$"
Local Const $dv = "(?i)^RogueKiller.*\.exe$"
Local Const $dw = "(?i)^RogueKiller_portable(32|64)\.exe$"
Local Const $dx[1][3] = [[$dq, $dv, False]]
Local Const $dy[1][2] = [[$dq, "RogueKiller Anti-Malware"]]
Local Const $dz[1][4] = [[$dq, "HKLM" & $7x & "\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall", $dt, "DisplayName"]]
Local Const $e0[1][5] = [[$dq, 'file', $e5, $du, False]]
Local Const $e6[1][5] = [[$dq, 'folder', Null, $dt, True]]
Local Const $e7[1][5] = [[$dq, 'file', Null, $dw, False]]
_vv($cb, $dx)
_vv($ci, $dy)
_vv($cj, $dz)
_vv($cf, $e6)
...
_114()
Func _115()
Local Const $dq = "adwcleaner"
Local Const $cx = "(?i)^AdwCleaner"
Local Const $e4 = "(?i)^Malwarebytes"
Local Const $dt = "(?i)^AdwCleaner.*\.exe$"
Dim $cb
Dim $cc
Dim $ce
Dim $cg
Local Const $dx[1][3] = [[$dq, $dt, False]]
Local Const $dy[1][5] = [[$dq, 'file', $e4, $dt, False]]
Local Const $dz[1][5] = [[$dq, 'folder', Null, $cx, True]]
_vv($cb, $dx)
_vv($cc, $dy)
_vv($ce, $dy)
_vv($cg, $dz)
...
Func _116()
Local Const $e2 = Null
Local Const $dq = "zhpcleaner"
Dim $cb
Dim $cc
Dim $ce
Local Const $dt = "(?i)^ZHPCleaner.*\.exe$"
Local Const $du = "(?i)^ZHPCleaner.*\.(exe|txt|lnk)$"
Local Const $dx[1][3] = [[$dq, $dt, False]]
Local Const $dy[1][5] = [[$dq, 'file', $e2, $du, False]]
_vv($cb, $dx)
_vv($cc, $dy)
...
Dim $cg
Dim $cf
Local Const $cx = "(?i)^UsbFix"
Local Const $e4 = "(?i)^SosVirus"
Local Const $dt = "(?i)^UsbFix.*\.(exe|lnk|txt)$"
Local Const $du = "(?i)^Un-UsbFix\.exe$"
Local Const $dv = "(?i)^UsbFixQuarantine$"
Local Const $dw = "(?i)^UsbFix.*\.exe$"
Local Const $e8[1][3] = [[$dq, $dw, False]]
Local Const $dx[1][2] = [[$dq, $cx]]
Local Const $dy[1][3] = [[$dq, $cx, $du]]
Local Const $dz[1][5] = [[$dq, 'file', $e4, $dt, False]]
Local Const $e0[1][5] = [[$dq, 'folder', Null, $dv, True]]
Local Const $e6[1][5] = [[$dq, 'folder', Null, $cx, False]]
_vv($cb, $e8)
_vv($cp, $dy)
_vv($cc, $dz)
_vv($cd, $dz)
...
Dim $cd
Dim $ch
Local Const $cx = "(?i)^AdsFix"
Local Const $e4 = "(?i)^SosVirus"
Local Const $dt = "(?i)^AdsFix.*\.exe$"
Local Const $du = "(?i)^AdsFix.*\.(exe|txt|lnk)$"
Local Const $dv = "(?i)^AdsFix.*\.txt$"
Local Const $dx[1][3] = [[$dq, $dt, False]]
Local Const $dy[1][5] = [[$dq, 'file', $e4, $du, False]]
Local Const $dz[1][5] = [[$dq, 'file', Null, $dv, False]]
Local Const $e0[1][5] = [[$dq, 'folder', Null, $cx, True]]
Local Const $e6[1][2] = [[$dq, $cx]]
_vv($cb, $dx)
_vv($cc, $dy)
_vv($cd, $dy)
_vv($ce, $dy)
...
_118()
Func _119()
Local Const $dq = "aswmbr"
Dim $cb
Dim $cc
Dim $ce
Dim $cq
Local Const $cx = "(?i)^avast"
Local Const $dt = "(?i)^aswmbr.*\.(exe|txt|lnk)$"
Local Const $du = "(?i)^MBR\.dat$"
Local Const $dv = "(?i)^aswmbr.*\.exe$"
Local $7x = ""
If @OSArch = "X64" Then $7x = "64"
Local Const $dx[1][3] = [[$dq, $dv, True]]
Local Const $dy[1][5] = [[$dq, 'file', $cx, $dt, False]]
Local Const $dz[1][5] = [[$dq, 'file', Null, $du, False]]
Local Const $e0[1][3] = [[$dq, "HKLM" & $7x & "\SYSTEM\CurrentControlSet\Enum\Root\LEGACY_ASWMBR", True]]
_vv($cb, $dx)
_vv($cc, $dy)
_vv($cc, $dz)
_vv($ce, $dy)
...
Dim $ce
Local Const $cx = "(?i)^Farbar"
Local Const $dt = "(?i)^FSS.*\.(exe|txt|lnk)$"
Local Const $du = "(?i)^FSS.*\.exe$"
Local Const $dx[1][3] = [[$dq, $du, False]]
Local Const $dy[1][5] = [[$dq, 'file', $cx, $dt, False]]
_vv($cb, $dx)
_vv($cc, $dy)
_vv($ce, $dy)
...
_11a()
Func _11b()
Local Const $dq = "toolsdiag"
Dim $cb
Dim $cc
Dim $ce
Dim $cg
Local Const $dt = "(?i)^toolsdiag.*\.exe$"
Local Const $du = "(?i)^ToolsDiag$"
Local Const $dx[1][5] = [[$dq, 'folder', Null, $du, False]]
Local Const $dy[1][5] = [[$dq, 'file', Null, $dt, False]]
Local Const $dz[1][3] = [[$dq, $dt, False]]
_vv($cb, $dz)
_vv($cc, $dy)
_vv($ce, $dy)
_vv($cg, $dx)
...
_11b()
Func _11c()
Local Const $dq = "scanrapide"
Dim $cg
Dim $cc
Dim $ce
Local Const $cx = Null
Local Const $dt = "(?i)^ScanRapide.*\.exe$"
Local Const $du = "(?i)^ScanRapide\[R[0-9]+\]\.txt$"
Local Const $dx[1][5] = [[$dq, 'file', $cx, $dt, False]]
Local Const $dy[1][5] = [[$dq, 'file', Null, $du, False]]
_vv($cc, $dx)
_vv($ce, $dx)
_vv($cg, $dy)
EndFunc
...
Dim $cg
Dim $ch
Local Const $cx = "(?i)^OldTimer"
Local Const $dt = "(?i)^OTL.*\.exe$"
Local Const $du = "(?i)^OTL.*\.(exe|txt)$"
Local Const $dv = "(?i)^Extras\.txt$"
Local Const $dw = "(?i)^_OTL$"
Local Const $e9 = "(?i)^OldTimer Tools$"
Local Const $dx[1][3] = [[$dq, $dt, False]]
Local Const $dy[1][5] = [[$dq, 'file', $cx, $du, False]]
Local Const $dz[1][5] = [[$dq, 'file', Null, $dv, False]]
Local Const $e0[1][5] = [[$dq, 'folder', Null, $dw, True]]
Local Const $e6[1][2] = [[$dq, $e9]]
_vv($cb, $dx)
_vv($cc, $dy)
_vv($cc, $dz)
_vv($ce, $dy)
...
Dim $cg
Local Const $cx = "(?i)^OldTimer"
Local Const $dt = "(?i)^OTM.*\.exe$"
Local Const $du = "(?i)^_OTM$"
Local Const $dx[1][3] = [[$dq, $dt, False]]
Local Const $dy[1][5] = [[$dq, 'file', $cx, $dt, False]]
Local Const $dz[1][5] = [[$dq, 'folder', Null, $du, True]]
_vv($cb, $dx)
_vv($cc, $dy)
_vv($ce, $dy)
_vv($cg, $dz)
...
_11e()
Func _11f()
Local Const $dq = "listparts"
Dim $cb
Dim $cc
Dim $ce
Dim $cg
Local Const $cx = "(?i)^Farbar"
Local Const $dt = "(?i)^listParts.*\.exe$"
Local Const $du = "(?i)^Results\.txt$"
Local Const $dx[1][3] = [[$dq, $dt, False]]
Local Const $dy[1][5] = [[$dq, 'file', $cx, $dt, False]]
Local Const $dz[1][5] = [[$dq, 'file', Null, $du, False]]
_vv($cb, $dx)
...
_11f()
Func _11g()
Local Const $dq = "minitoolbox"
Dim $cb
Dim $cc
Dim $ce
Dim $cg
Local Const $cx = "(?i)^Farbar"
Local Const $dt = "(?i)^MiniToolBox.*\.exe$"
Local Const $du = "(?i)^MTB\.txt$"
Local Const $dx[1][2] = [[$dq, $dt]]
Local Const $dy[1][5] = [[$dq, 'file', $cx, $dt, False]]
Local Const $dz[1][5] = [[$dq, 'file', Null, $du, False]]
_vv($cb, $dx)
_vv($cc, $dy)
...
_11g()
Func _11h()
Local Const $dq = "miniregtool"
Dim $cb
Dim $cc
Dim $ce
Dim $cg
Local Const $cx = Null
Local Const $dt = "(?i)^MiniRegTool.*\.exe$"
Local Const $du = "(?i)^MiniRegTool.*\.(exe|zip)$"
Local Const $dv = "(?i)^Result\.txt$"
Local Const $dw = "(?i)^MiniRegTool"
Local Const $dx[1][3] = [[$dq, $dt, False]]
Local Const $dy[1][5] = [[$dq, 'file', $cx, $du, False]]
Local Const $dz[1][5] = [[$dq, 'file', $cx, $dv, False]]
Local Const $e0[1][5] = [[$dq, 'folder', $cx, $dw, False]]
_vv($cb, $dx)
_vv($cc, $dy)
_vv($cc, $dz)
_vv($cc, $e0)
...
_11h()
Func _11i()
Local Const $dq = "grantperms"
Dim $cb
Dim $cc
Dim $ce
Dim $cg
Local Const $cx = Null
Local Const $dt = "(?i)^GrantPerms.*\.exe$"
Local Const $du = "(?i)^GrantPerms.*\.(exe|zip)$"
Local Const $dv = "(?i)^Perms\.txt$"
Local Const $dw = "(?i)^GrantPerms"
Local Const $dx[1][3] = [[$dq, $dt, False]]
Local Const $dy[1][5] = [[$dq, 'file', $cx, $du, False]]
Local Const $dz[1][5] = [[$dq, 'file', $cx, $dv, False]]
Local Const $e0[1][5] = [[$dq, 'folder', $cx, $dw, False]]
...
Dim $cb
Dim $cq
Local Const $cx = "(?i)^Swearware"
Local Const $dt = "(?i)^Combofix.*\.exe$"
Local Const $du = "(?i)^CFScript\.txt$"
Local Const $dv = "(?i)^Qoobox$"
Local Const $dw = "(?i)^Combofix.*\.txt$"
Local Const $e9 = "(?i)^(grep|PEV|NIRCMD|MBR|SED|SWREG|SWSC|SWXCACLS|Zip)\.exe$"
Local Const $ea = "(?i)^Swearware$"
Local $7x = ""
If @OSArch = "X64" Then $7x = "64"
Local Const $dx[1][5] = [[$dq, 'file', $cx, $dt, False]]
Local Const $dy[1][5] = [[$dq, 'file', Null, $du, False]]
Local Const $dz[1][5] = [[$dq, 'folder', Null, $dv, True]]
Local Const $e0[1][5] = [[$dq, 'file', Null, $dw, False]]
Local Const $e6[1][5] = [[$dq, 'file', Null, $e9, True]]
Local Const $e7[1][2] = [[$dq, $ea]]
Local Const $eb[1][3] = [[$dq, $dt, True]]
Local Const $ec[1][3] = [[$dq, "HKLM" & $7x & "\SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\combofix.exe", False]]
_vv($cc, $dx)
_vv($cc, $dy)
_vv($ce, $dx)
_vv($ce, $dy)
...
_11j()
Func _11k()
Local Const $dq = "regtoolexport"
Dim $cb
Dim $cc
Dim $ce
Local Const $cx = Null
Local Const $dt = "(?i)^regtoolexport.*\.exe$"
Local Const $du = "(?i)^Export.*\.reg$"
Local Const $dx[1][3] = [[$dq, $dt, False]]
Local Const $dy[1][5] = [[$dq, 'file', $cx, $dt, False]]
Local Const $dz[1][5] = [[$dq, 'file', $cx, $du, False]]
_vv($cb, $dx)
_vv($cc, $dy)
_vv($cc, $dz)
_vv($ce, $dy)
...
_11k()
Func _11l()
Local Const $dq = "tdsskiller"
Dim $cb
Dim $cc
Dim $ce
Dim $cg
Local Const $cx = "(?i)^.*Kaspersky"
Local Const $dt = "(?i)^tdsskiller.*\.exe$"
Local Const $du = "(?i)^tdsskiller.*\.(exe|zip)$"
Local Const $dv = "(?i)^TDSSKiller.*_log\.txt$"
Local Const $dw = "(?i)^TDSSKiller"
Local Const $dx[1][3] = [[$dq, $dt, True]]
Local Const $dy[1][5] = [[$dq, 'file', $cx, $du, False]]
Local Const $dz[1][5] = [[$dq, 'file', Null, $dv, False]]
Local Const $e0[1][5] = [[$dq, 'folder', Null, $dw, False]]
_vv($cb, $dx)
_vv($cc, $dy)
_vv($cc, $e0)
_vv($ce, $dy)
...
_11l()
Func _11m()
Local Const $dq = "winupdatefix"
Dim $cb
Dim $cc
Dim $ce
Dim $cg
Local Const $cx = Null
Local Const $dt = "(?i)^winupdatefix.*\.exe$"
Local Const $du = "(?i)^winupdatefix.*\.txt$"
Local Const $dx[1][3] = [[$dq, $dt, False]]
Local Const $dy[1][5] = [[$dq, 'file', $cx, $dt, False]]
Local Const $dz[1][5] = [[$dq, 'file', $cx, $du, False]]
_vv($cb, $dx)
...
_11m()
Func _11n()
Local Const $dq = "rsthosts"
Dim $cb
Dim $cc
Dim $ce
Dim $cg
Local Const $cx = Null
Local Const $dt = "(?i)^rsthosts.*\.exe$"
Local Const $du = "(?i)^RstHosts.*\.txt$"
Local Const $dx[1][3] = [[$dq, $dt, False]]
Local Const $dy[1][5] = [[$dq, 'file', $cx, $dt, Null]]
Local Const $dz[1][5] = [[$dq, 'file', $cx, $du, Null]]
_vv($cb, $dx)
_vv($cc, $dy)
_vv($ce, $dy)
_vv($cg, $dz)
...
_11n()
Func _11o()
Local Const $dq = "winchk"
Dim $cb
Dim $cc
Dim $ce
Dim $cg
Local Const $cx = Null
Local Const $dt = "(?i)^winchk.*\.exe$"
Local Const $du = "(?i)^WinChk.*\.txt$"
Local Const $dx[1][3] = [[$dq, $dt, False]]
Local Const $dy[1][5] = [[$dq, 'file', $cx, $dt, False]]
Local Const $dz[1][5] = [[$dq, 'file', $cx, $du, False]]
_vv($cb, $dx)
...
_11o()
Func _11p()
Local Const $dq = "avenger"
Dim $cb
Dim $cc
Dim $ce
Dim $cg
Local Const $cx = Null
Local Const $dt = "(?i)^avenger.*\.(exe|zip)$"
Local Const $du = "(?i)^avenger"
Local Const $dv = "(?i)^avenger.*\.txt$"
Local Const $dw = "(?i)^avenger.*\.exe$"
Local Const $dx[1][3] = [[$dq, $dw, False]]
Local Const $dy[1][5] = [[$dq, 'file', $cx, $dt, False]]
Local Const $dz[1][5] = [[$dq, 'folder', $cx, $du, False]]
Local Const $e0[1][5] = [[$dq, 'file', $cx, $dv, False]]
_vv($cb, $dx)
_vv($cc, $dy)
_vv($cc, $dz)
_vv($ce, $dy)
...
Dim $cd
Dim $ch
Local Const $cx = "(?i)^Emsi"
Local Const $dt = "(?i)^BlitzBlank.*\.exe$"
Local Const $du = "(?i)^BlitzBlank.*\.log$"
Local Const $dx[1][3] = [[$dq, $dt, False]]
Local Const $dy[1][5] = [[$dq, 'file', $cx, $dt, False]]
Local Const $dz[1][5] = [[$dq, 'file', Null, $du, False]]
_vv($cb, $dx)
...
Dim $ch
Local Const $cx = Null
Local Const $dt = "(?i)^zoek.*\.exe$"
Local Const $du = "(?i)^zoek.*\.log$"
Local Const $dv = "(?i)^zoek"
Local Const $dw = "(?i)^runcheck.*\.txt$"
Local Const $dx[1][3] = [[$dq, $dt, False]]
Local Const $dy[1][5] = [[$dq, 'file', $cx, $dt, False]]
Local Const $dz[1][5] = [[$dq, 'file', $cx, $du, False]]
Local Const $e0[1][5] = [[$dq, 'folder', $cx, $dv, True]]
Local Const $e6[1][5] = [[$dq, 'file', $cx, $dw, False]]
_vv($cb, $dx)
_vv($cc, $dy)
_vv($ce, $dy)
_vv($cg, $dz)
...
_11r()
Func _11s()
Local Const $dq = "remediate-vbs-worm"
Dim $cb
Dim $cc
Dim $ce
Dim $cg
Dim $cd
Dim $ch
Local Const $cx = "(?i).*VBS autorun worms.*"
Local Const $e4 = Null
Local Const $dt = "(?i)^remediate.?vbs.?worm.*\.exe$"
Local Const $du = "(?i)^Rem-VBS.*\.log$"
Local Const $dv = "(?i)^Rem-VBS"
Local Const $dw = "(?i)^Rem-VBSworm.*\.exe$"
Local Const $dx[1][3] = [[$dq, $dt, False]]
Local Const $dy[1][5] = [[$dq, 'file', $e4, $dt, False]]
Local Const $dz[1][5] = [[$dq, 'file', $e4, $du, False]]
Local Const $e0[1][5] = [[$dq, 'folder', $cx, $dv, True]]
Local Const $e6[1][2] = [[$dq, $dw]]
Local Const $e7[1][5] = [[$dq, 'file', $e4, $dw, False]]
_vv($cb, $dx)
_vv($cb, $e6)
_vv($cc, $dy)
_vv($cc, $e7)
...
_11s()
Func _11t()
Local Const $dq = "ckscanner"
Dim $cb
Dim $cc
Dim $ce
Local Const $cx = Null
Local Const $dt = "(?i)^CKScanner.*\.exe$"
Local Const $du = "(?i)^CKfiles.*\.txt$"
Local Const $dx[1][3] = [[$dq, $dt, False]]
Local Const $dy[1][5] = [[$dq, 'file', $cx, $dt, False]]
Local Const $dz[1][5] = [[$dq, 'file', $cx, $du, False]]
_vv($cb, $dx)
...
_11t()
Func _11u()
Local Const $dq = "quickdiag"
Dim $cb
Dim $cc
Dim $ce
Dim $cg
Local Const $cx = "(?i)^SosVirus"
Local Const $dt = "(?i)^QuickDiag.*\.exe$"
Local Const $du = "(?i)^QuickDiag.*\.(exe|txt)$"
Local Const $dv = "(?i)^QuickScript.*\.txt$"
Local Const $dw = "(?i)^QuickDiag.*\.txt$"
Local Const $e9 = "(?i)^QuickDiag"
Local Const $dx[1][3] = [[$dq, $dt, False]]
Local Const $dy[1][5] = [[$dq, 'file', $cx, $du, True]]
Local Const $dz[1][5] = [[$dq, 'file', Null, $dv, True]]
Local Const $e0[1][5] = [[$dq, 'file', Null, $dw, True]]
Local Const $e6[1][5] = [[$dq, 'folder', Null, $e9, True]]
_vv($cb, $dx)
_vv($cc, $dy)
_vv($ce, $dy)
_vv($cc, $dz)
...
Local $7x = ""
If @OSArch = "X64" Then $7x = "64"
Local Const $ed = "(?i)^Adlice Diag"
Local Const $dt = "(?i)^Diag version"
Local Const $du = "(?i)^Diag$"
Local Const $dv = "(?i)^ADiag$"
Local Const $dw = "(?i)^Diag_portable(32|64)\.exe$"
Local Const $e9 = "(?i)^Diag\.lnk$"
Local Const $ea = "(?i)^Diag_setup\.exe$"
Local Const $ee = "(?i)^Diag(32|64)?\.exe$"
Local Const $dx[1][3] = [[$dq, $ed, False]]
Local Const $dy[1][4] = [[$dq, "HKLM" & $7x & "\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall", $dt, "DisplayName"]]
Local Const $dz[1][5] = [[$dq, 'folder', Null, $du, True]]
Local Const $e0[1][5] = [[$dq, 'folder', Null, $dv, True]]
Local Const $e6[1][5] = [[$dq, 'file', Null, $dw, False]]
Local Const $e7[1][5] = [[$dq, 'file', Null, $e9, False]]
Local Const $eb[1][5] = [[$dq, 'file', Null, $ea, False]]
Local Const $ec[1][2] = [[$dq, $ee]]
_vv($cb, $dx)
_vv($cb, $ec)
_vv($cj, $dy)
_vv($cf, $dz)
...
Func _11w()
Local Const $e2 = Null
Local Const $dq = "rstassociations"
Dim $cb
Dim $cc
Dim $ce
Dim $cg
Local Const $dt = "(?i)^rstassociations.*\.(exe|scr)$"
Local Const $du = "(?i)^RstAssociations.*\.txt$"
Local Const $dx[1][3] = [[$dq, $dt, False]]
Local Const $dy[1][5] = [[$dq, 'file', $e2, $dt, False]]
Local Const $dz[1][5] = [[$dq, 'file', $e2, $du, False]]
_vv($cb, $dx)
_vv($cc, $dy)
_vv($ce, $dy)
_vv($cg, $dz)
...
Dim $cc
Dim $ce
Local Const $dt = "(?i)^SFT.*\.exe$"
Local Const $du = "(?i)^SFT.*\.(txt|exe|zip)$"
Local Const $dx[1][3] = [[$dq, $dt, False]]
Local Const $dy[1][5] = [[$dq, 'file', $e2, $du, False]]
_vv($cb, $dx)
_vv($cc, $dy)
...
_11x()
Func _11y()
Local Const $dq = "logonfix"
Dim $cb
Dim $cc
Dim $ce
Dim $cg
Local Const $cx = Null
Local Const $dt = "(?i)^logonfix.*\.exe$"
Local Const $du = "(?i)^LogonFix.*\.txt$"
Local Const $dx[1][3] = [[$dq, $dt, False]]
Local Const $dy[1][5] = [[$dq, 'file', $cx, $dt, False]]
Local Const $dz[1][5] = [[$dq, 'file', $cx, $du, False]]
_vv($cb, $dx)
...
_11y()
Func _11z()
Local Const $dq = "cmd-command"
Dim $cb
Dim $cc
Dim $ce
Local Const $cx = "(?i)^g3n-h@ckm@n$"
Local Const $dt = "(?i)^cmd-command.*\.exe$"
Local Const $dx[1][3] = [[$dq, $dt, False]]
Local Const $dy[1][5] = [[$dq, 'file', $cx, $dt, False]]
_vv($cb, $dx)
_vv($cc, $dy)
...
_11z()
Func _120()
Local Const $dq = "report_chkdsk"
Dim $cb
Dim $cc
Dim $ce
Local Const $cx = Null
Local Const $dt = "(?i)^Report_CHKDSK.*\.exe$"
Local Const $du = "(?i)^RapportCHK.*\.txt$"
Local Const $dx[1][3] = [[$dq, $dt, False]]
Local Const $dy[1][5] = [[$dq, 'file', $cx, $dt, False]]
Local Const $dz[1][5] = [[$dq, 'file', $cx, $du, False]]
_vv($cb, $dx)
...
Dim $cg
Dim $cf
Local Const $cx = "(?i)^C_XX$"
Local Const $ef = "(?i)^SEAF$"
Local Const $dt = "(?i)^seaf.*\.exe$"
Local Const $du = "(?i)^Un-SEAF\.exe$"
Local Const $dv = "(?i)^SeafLog.*\.txt$"
Local $7x = ""
If @OSArch = "X64" Then $7x = "64"
Local Const $dx[1][3] = [[$dq, $dt, False]]
Local Const $dy[1][5] = [[$dq, 'file', $cx, $dt, False]]
Local Const $dz[1][3] = [[$dq, $ef, $du]]
Local Const $e0[1][3] = [[$dq, "HKLM" & $7x & "\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\SEAF", False]]
Local Const $e6[1][5] = [[$dq, 'file', Null, $dv, False]]
Local Const $e7[1][5] = [[$dq, 'folder', Null, $ef, True]]
_vv($cb, $dx)
_vv($cc, $dy)
_vv($ce, $dy)
_vv($cp, $dz)
...
Local Const $dq = "dds"
Local Const $cx = "(?i)^Swearware"
Local Const $dt = "(?i)dds.*\.com"
Local Const $du = "(?i)(dds|attach).*\.txt"
Local Const $dx[1][3] = [[$dq, $dt, False]]
Local Const $dy[1][5] = [[$dq, 'file', $cx, $dt, False]]
Local Const $dz[1][5] = [[$dq, 'file', Null, $du, False]]
_vv($cb, $dx)
...
Dim $cc
Dim $ce
Local Const $dq = "defogger"
Local Const $cx = Null
Local Const $dt = "(?i)defogger.*\.exe"
Local Const $du = "(?i)defogger.*\.(log|exe)"
Local Const $dx[1][3] = [[$dq, $dt, False]]
Local Const $dy[1][5] = [[$dq, 'file', $cx, $du, False]]
_vv($cb, $dx)
_vv($cc, $dy)
...
Dim $ce
Dim $cg
Local Const $dq = "javara"
Local Const $cx = "(?i)^The RaProducts Team"
Local Const $dt = "(?i)Javara"
Local Const $du = "(?i)Javara.*\.exe"
Local Const $dv = "(?i)Javara.*\.(zip|exe)"
Local Const $dw = "(?i)Javara.*\.log"
Local Const $dx[1][3] = [[$dq, $du, False]]
Local Const $dy[1][5] = [[$dq, 'file', $cx, $dv, False]]
Local Const $dz[1][5] = [[$dq, 'file', Null, $dw, False]]
Local Const $e0[1][5] = [[$dq, 'folder', Null, $dt, False]]
_vv($cb, $dx)
_vv($cc, $dy)
_vv($cc, $e0)
_vv($ce, $dy)
...
_124()
Func _125()
Local Const $dq = "g3n-h@ckm@n_tools"
Dim $ch
Local Const $ea = "(?i)^g3n-h@ckm@n$"
Local Const $e7[1][2] = [[$dq, $ea]]
_vv($ch, $e7)
EndFunc
_125()
Func _126()
Local Const $dq = "systemlook"
Dim $cb
Dim $cc
Dim $ce
Local Const $cx = Null
Local Const $dt = "(?i)^SystemLook.*\.exe$"
Local Const $du = "(?i)^SystemLook.*\.(exe|txt)$"
Local Const $dx[1][3] = [[$dq, $dt, False]]
Local Const $dy[1][5] = [[$dq, 'file', $cx, $du, False]]
_vv($cb, $dx)
_vv($cc, $dy)
...
Local $ej = False
Local Const $ek = _zu(@AppDataDir & "\ZHP")
For $el In $b6
Local $em = $b6.Item($el)
Local $en = False
...
Local Const $ev = GUICreate($bx, 500, 195, 202, 112)
Local Const $ew = GUICtrlCreateGroup("Actions", 8, 8, 400, 153)
Local Const $ex = GUICtrlCreateCheckbox($7e, 16, 40, 129, 17)
Local Const $ey = GUICtrlCreateCheckbox($7f, 16, 80, 190, 17)
Local Const $ez = GUICtrlCreateCheckbox($7g, 16, 120, 190, 17)
Local Const $f0 = GUICtrlCreateCheckbox($7h, 220, 40, 137, 17)
Local Const $f1 = GUICtrlCreateCheckbox($7i, 220, 80, 137, 17)
Local Const $f2 = GUICtrlCreateCheckbox($7j, 220, 120, 180, 17)
Global $bk = GUICtrlCreateProgress(8, 170, 480, 17)
GUICtrlCreateGroup("", -99, -99, 1, 1)
GUICtrlSetState($ex, 1)
Local Const $f3 = GUICtrlCreatePic(@TempDir & "\kprm-logo.gif", 415, 16, 76, 76)
Local Const $f4 = GUICtrlCreateButton($7k, 415, 120, 75, 40)
GUISetState(@SW_SHOW)
While 1
Local $f5 = GUIGetMsg()
Switch $f5
...
WEnd
Func _128()
Local Const $f6 = @HomeDrive & "\KPRM"
If Not FileExists($f6) Then
DirCreate($f6)
EndIf
If Not FileExists($f6) Then
MsgBox(16, $7m, $7n)
