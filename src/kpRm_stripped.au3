...
Local $ax = $at & "\" & $aw
Local $6s = RegRead($ax, $av)
If StringRegExp($6s, $au) Then
Return $ax
EndIf
WEnd
Return Null
...
EndFunc
Func _zx($b1)
If StringRegExp($b1, "^(HKLM|HKCU|HKU|HKCR|HKCC)64") Then
Local $av = StringReplace($b1, "64", "", 1)
Return $av
EndIf
Return $b1
EndFunc
...
EndFunc
Func _100($b9, $ba)
If $b9 = Null Or $b9 = "" Then Return
Local $bb = ProcessExists($b9)
If $bb <> 0 Then
_zm("     [X] Process " & $b9 & " not killed, it is possible that the deletion is not complete (" & $ba & ")")
Else
_zm("     [OK] Process " & $b9 & " killed (" & $ba & ")")
EndIf
EndFunc
Func _101($bc, $ba)
If $bc = Null Or $bc = "" Then Return
Local $bd = "[X]"
RegEnumVal($bc, "1")
If @error >= 0 Then
$bd = "[OK]"
EndIf
_zm("     " & $bd & " " & _zx($bc) & " deleted (" & $ba & ")")
EndFunc
Func _102($bc, $ba)
If $bc = Null Or $bc = "" Then Return
Local $78 = "", $79 = "", $5x = "", $7a = ""
Local $be = _xe($bc, $78, $79, $5x, $7a)
If $7a = ".exe" Then
Local $bf = $be[1] & $be[2]
Local $bd = "[OK]"
If FileExists($bf) Then
$bd = "[X]"
EndIf
_zm("     " & $bd & " Uninstaller run correctly (" & $ba & ")")
EndIf
EndFunc
Func _103($bc, $ba)
If $bc = Null Or $bc = "" Then Return
...
Return Null
EndIf
Local $bn[1][3] = [[Null, Null, Null]]
For $4g = 1 To $bl[0][0]
Local $bb = _y4($bl[$4g][0])
$bm += $bb
If $bb = 1 Then
_zm("    => [OK] RP named " & $bl[$4g][1] & " created at " & $bl[$4g][2] & " deleted")
Else
Local $bo[1][3] = [[$bl[$4g][0], $bl[$4g][1], $bl[$4g][2]]]
_vv($bn, $bo)
EndIf
Next
If 1 < UBound($bn) Then
Sleep(3000)
For $4g = 1 To UBound($bn) - 1
Local $bb = _y4($bn[$4g][0])
$bm += $bb
If $bb = 1 Then
_zm("    => [OK] RP named " & $bn[$4g][1] & " created at " & $bl[$4g][2] & " deleted")
Else
_zm("    => [X] RP named " & $bn[$4g][1] & " created at " & $bl[$4g][2] & " deleted")
EndIf
Next
EndIf
If $bl[0][0] = $bm Then
...
EndFunc
Func _108($bp)
Local $bq = StringLeft($bp, 4)
Local $br = StringMid($bp, 6, 2)
Local $bs = StringMid($bp, 9, 2)
Local $bt = StringRight($bp, 8)
Return $br & "/" & $bs & "/" & $bq & " " & $bt
EndFunc
Func _109($bu = False)
Local Const $bl = _y2()
...
Return Null
EndIf
Local Const $bv = _108(_31('n', -1470, _3p()))
Local $bw = False
Local $bx = False
Local $by = False
For $4g = 1 To $bl[0][0]
...
Local $bb = _y4($bl[$4g][0])
If $bb = 1 Then
_zm("    => [OK] RP named " & $bl[$4g][1] & " created at " & $bz & " deleted")
ElseIf $bu = False Then
$bw = True
Else
_zm("    => [X] RP named " & $bl[$4g][1] & " created at " & $bz & " deleted")
EndIf
EndIf
Next
If $bw = True Then
...
Local Const $c2 = @HomeDrive & "\KPRM"
Local Const $c3 = $c2 & "\regedit-backup-" & @YEAR & @MON & @MDAY & @HOUR & @MIN & ".reg"
If FileExists($c3) Then
FileMove($c3, $c3 & ".old")
EndIf
Local Const $c4 = RunWait("Regedit /e " & $c3)
If Not FileExists($c3) Or @error <> 0 Then
_zm("  [X] Failed to create registry backup")
MsgBox(16, $7m, $7n)
_zi()
Else
_zm("  [OK] Registry Backup: " & $c3)
EndIf
EndFunc
Func _10e()
...
Local $bb = RunWait(@ComSpec & " /c " & "ipconfig /flushdns", @TempDir, @SW_HIDE)
If @error <> 0 Then
_zm("  [X] Flush DNS")
Else
_zm("  [OK] Flush DNS")
EndIf
Local Const $c5[7] = [ "netsh winsock reset", "netsh winhttp reset proxy", "netsh winhttp reset tracing", "netsh winsock reset catalog", "netsh int ip reset all", "netsh int ipv4 reset catalog", "netsh int ipv6 reset catalog" ]
$bb = 0
For $4g = 0 To UBound($c5) -1
RunWait(@ComSpec & " /c " & $c5[$4g], @TempDir, @SW_HIDE)
If @error <> 0 Then
$bb += 1
EndIf
Next
...
_zm("  [X] Reset WinSock")
EndIf
Local $c6 = "HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced"
$bb = RegWrite($c6, "Hidden", "REG_DWORD", "2")
If $bb = 1 Then
_zm("  [OK] Hide Hidden file.")
Else
_zm("  [X] Hide Hidden File")
EndIf
$bb = RegWrite($c6, "HideFileExt", "REG_DWORD", "0")
If $bb = 1 Then
_zm("  [OK] Show Extensions for known file types")
Else
_zm("  [X] Show Extensions for known file types")
EndIf
$bb = RegWrite($c6, "ShowSuperHidden", "REG_DWORD", "0")
If $bb = 1 Then
...
Local Const $c7[88] = [ "AdliceDiag", "AdsFix", "AdwCleaner", "AHK_NavScan", "AlphaDecrypter", "AswMBR", "AuroraDecrypter", "Avast Decryptor Cryptomix", "Avenger", "BlitzBlank", "BitKangarooDecrypter", "BitStakDecrypter", "BTCWareDecrypter", "Check Browsers LNK", "CKScanner", "Clean_DNS", "ClearLNK", "CMD_Command", "Combofix", "Crypt38Decrypter", "CryptoSearch", "DDS", "DCryDecrypter", "Decrypt CryptON", "Defogger", "ESET Online Scanner", "FilesLockerDecrypter", "FixExec", "FRST", "FSS", "g3n-h@ckm@n tools", "GhostCryptDecrypter", "GibonDecrypter", "Grantperms", "HiddenTearDecrypter", "Hosts-perm", "InsaneCryptDecrypter", "JavaRa", "Junkware Removal Tool", "JigSawDecrypter", "ListCWall", "ListParts", "LogonFix", "Malwarebytes Anti-Rootkit", "MiniregTool", "Minitoolbox", "MirCopDecrypter", "OTL", "OTM", "PowerLockyDecrypter", "Pre_Scan", "ProcessClose", "QuickDiag", "Rakhni Decryptor", "Rannoh Decryptor", "RansomNoteCleaner", "RegtoolExport", "Remediate VBS Worm", "Report_CHKDSK", "Rkill", "RogueKiller", "RstAssociations", "RstHosts", "ScanRapide", "SEAF", "SecurityCheck", "SFT", "Shortcut Cleaner", "StrikedDecrypter", "StupidDecrypter", "Symantec Kovter Removal Tool", "Systemlook", "TDSSKiller", "ToolsDiag", "UnHide", "USB File Resc", "USBFix", "Unlock92Decrypter", "UnZacMe", "WinCHK", "WinUpdatefix", "ZHP Tools", "ZHPCleaner", "ZHPDiag", "ZHPFix", "ZHPLite", "Zoek"]
For $c8 = 0 To UBound($c7) - 1
Local $c9 = ObjCreate("Scripting.Dictionary")
Local $ca = ObjCreate("Scripting.Dictionary")
Local $cb = ObjCreate("Scripting.Dictionary")
Local $cc = ObjCreate("Scripting.Dictionary")
Local $cd = ObjCreate("Scripting.Dictionary")
$c9.add("key", $ca)
$c9.add("element", $cb)
$c9.add("uninstall", $cc)
$c9.add("process", $cd)
...
Global $cj[1][5] = [[Null, Null, Null, Null, Null]]
Global $ck[1][2] = [[Null, Null]]
Global $cl[1][2] = [[Null, Null]]
Global $cm[1][4] = [[Null, Null, Null, Null]]
Global $cn[1][5] = [[Null, Null, Null, Null, Null]]
Global $co[1][5] = [[Null, Null, Null, Null, Null]]
Global $cp[1][5] = [[Null, Null, Null, Null, Null]]
Global $cq[1][5] = [[Null, Null, Null, Null, Null]]
Global $cr[1][5] = [[Null, Null, Null, Null, Null]]
Global $cs[1][3] = [[Null, Null, Null]]
Global $ct[1][3] = [[Null, Null, Null]]
Global $cu[1][4] = [[Null, Null, Null, Null]]
Func _10g($at, $cv = 0, $cw = False)
If $cw Then
_yx($at)
_yf($at)
EndIf
Local Const $cx = FileGetAttrib($at)
If StringInStr($cx, "R") Then
FileSetAttrib($at, "-R", $cv)
EndIf
If StringInStr($cx, "S") Then
FileSetAttrib($at, "-S", $cv)
EndIf
If StringInStr($cx, "H") Then
FileSetAttrib($at, "-H", $cv)
EndIf
If StringInStr($cx, "A") Then
FileSetAttrib($at, "-A", $cv)
EndIf
EndFunc
Func _10h($cy, $b3, $cz = Null, $cw = False)
Local Const $d0 = _zt($cy)
If $d0 Then
If $cz And StringRegExp($cy, "(?i)\.(exe|com)$") Then
Local Const $d1 = FileGetVersion($cy, "CompanyName")
If @error Then
Return 0
ElseIf Not StringRegExp($d1, $cz) Then
Return 0
EndIf
EndIf
_0zz($b3, 'element', $cy)
_10g($cy, 0, $cw)
Local $d2 = FileDelete($cy)
If $d2 Then
Return 1
EndIf
Return 2
...
Return 0
EndFunc
Func _10i($at, $b3, $cw = False)
Local $d0 = _zu($at)
If $d0 Then
_0zz($b3, 'element', $at)
_10g($at, 1, $cw)
Local Const $d2 = DirRemove($at, $q)
If $d2 Then
Return 1
EndIf
Return 2
...
EndFunc
Func _10j($at, $cy, $d3)
Local Const $d4 = $at & "\" & $cy
Local Const $5z = FileFindFirstFile($d4)
Local $ah = []
If $5z = -1 Then
Return $ah
EndIf
...
EndIf
Local $78 = "", $79 = "", $5x = "", $7a = ""
Local $be = _xe($d5, $78, $79, $5x, $7a)
Local $cy = $5x & $7a
For $d8 = 1 To UBound($d6) - 1
If $d6[$d8][3] And $d7 = $d6[$d8][1] And StringRegExp($cy, $d6[$d8][3]) Then
Local $bb = 0
Local $cw = False
If $d6[$d8][4] = True Then
$cw = True
EndIf
If $d7 = 'file' Then
$bb = _10h($d5, $d6[$d8][0], $d6[$d8][2], $cw)
ElseIf $d7 = 'folder' Then
$bb = _10i($d5, $d6[$d8][0], $cw)
EndIf
EndIf
Next
EndFunc
...
FileClose($5z)
EndFunc
Func _10n($da, $b3, $cw = False)
If $cw = True Then
_yx($da)
_yf($da, $8c)
EndIf
Local Const $bb = RegDelete($da)
If $bb <> 0 Then
_0zz($b3, "key", $da)
EndIf
Return $bb
...
Local $db = 50
Local $bb = Null
If 0 = ProcessExists($b9) Then Return 0
If $cw = True Then
_yt($b9)
If 0 = ProcessExists($b9) Then Return 0
EndIf
...
$db -= 1
Sleep(250)
Until($db = 0 Or 0 = ProcessExists($b9))
$bb = ProcessExists($b9)
If 0 = $bb Then
Return 1
EndIf
...
Local $dd = $dc[$4g][0]
Local $de = $dc[$4g][1]
For $db = 1 To UBound($74) - 1
If StringRegExp($dd, $74[$db][1]) Then
_10o($de, $74[$db][2])
_0zz($74[$db][0], "process", $dd)
EndIf
Next
Next
EndFunc
...
If @error <> 0 Then ExitLoop
For $df = 1 To UBound($74) - 1
If $aw And $74[$df][1] Then
If StringRegExp($aw, $74[$df][1]) Then
Local $dn = $dm[$59] & "\" & $aw
_10n($dn, $74[$df][0])
EndIf
EndIf
...
Func _10u($74)
For $4g = 1 To UBound($74) - 1
Local $dn = _zq($74[$4g][1], $74[$4g][2], $74[$4g][3])
If $dn And $dn <> "" Then
_10n($dn, $74[$4g][0])
EndIf
Next
...
Func _10v($74)
For $4g = 1 To UBound($74) - 1
_10n($74[$4g][1], $74[$4g][0], $74[$4g][2])
Next
EndFunc
Func _10w($74)
For $4g = 1 To UBound($74) - 1
If FileExists($74[$4g][1]) Then
Local $do = _x1($74[$4g][1])
If @error = 0 Then
For $dj = 1 To $do[0]
_10h($74[$4g][1] & '\' & $do[$dj], $74[$4g][0], $74[$4g][2], $74[$4g][3])
Next
EndIf
EndIf
Next
...
Local Const $dx[1][5] = [[$dp, 'file', $cz, $du, False]]
...
Dim $ck
Local $dp = "ZHP Tools"
Local Const $e0[1][2] = [[$dp, "(?i)^ZHP$"]]
Local Const $e1[1][5] = [[$dp, 'folder', Null, "(?i)^ZHP$", True]]
_vv($ck, $e0)
_vv($cp, $e1)
EndFunc
_10y()
...
Local Const $dx[1][5] = [[$e2, 'file', $cz, $dt, False]]
_vv($ce, $dw)
_vv($cf, $dx)
_vv($ch, $dx)
EndFunc
...
_111()
Func _112($e3 = False)
Local Const $cz = "(?i)^Malwarebytes Anti-Rootkit$"
...
Local Const $dx[1][2] = [[$dp, $cz]]
Local Const $dy[1][5] = [[$dp, 'file', $e4, $ds, False]]
Local Const $dz[1][5] = [[$dp, 'folder', $cz, $dt, False]]
Local Const $e5[1][4] = [[$dp, @AppDataCommonDir & "\Malwarebytes\Malwarebytes' Anti-Malware\Quarantine", Null, True]]
_vv($ce, $dw)
_vv($cf, $dy)
_vv($ch, $dy)
_vv($cf, $dz)
...
Local Const $dz[1][5] = [[$dp, 'file', $e6, $dt, False]]
Local Const $e5[1][5] = [[$dp, 'folder', Null, $ds, True]]
Local Const $e7[1][5] = [[$dp, 'file', Null, $dv, False]]
_vv($ce, $dw)
_vv($cl, $dx)
_vv($cm, $dy)
_vv($ci, $e5)
...
Dim $cj
Local Const $dw[1][3] = [[$dp, $ds, False]]
Local Const $dx[1][5] = [[$dp, 'file', $e4, $ds, False]]
Local Const $dy[1][5] = [[$dp, 'folder', Null, $cz, True]]
_vv($ce, $dw)
_vv($cf, $dx)
_vv($ch, $dx)
_vv($cj, $dy)
...
Local Const $e8[1][3] = [[$dp, $dv, False]]
Local Const $dw[1][2] = [[$dp, $cz]]
Local Const $dx[1][3] = [[$dp, $cz, $dt]]
Local Const $dy[1][5] = [[$dp, 'file', $e4, $ds, False]]
Local Const $dz[1][5] = [[$dp, 'folder', Null, $du, True]]
Local Const $e5[1][5] = [[$dp, 'folder', Null, $cz, False]]
_vv($ce, $e8)
_vv($cs, $dx)
_vv($cf, $dy)
_vv($cg, $dy)
...
Local Const $dw[1][5] = [[$dp, 'file', $cz, $ds, False]]
...
Local Const $dw[1][3] = [[$dp, $ds, False]]
Local Const $dx[1][5] = [[$dp, 'file', $cz, $dt, False]]
Local Const $dy[1][5] = [[$dp, 'file', $cz, $du, False]]
Local Const $dz[1][5] = [[$dp, 'folder', $cz, $dv, False]]
_vv($ce, $dw)
_vv($cf, $dx)
_vv($cf, $dy)
_vv($cf, $dz)
...
Local Const $e5[1][5] = [[$dp, 'file', Null, $e9, True]]
Local Const $e7[1][2] = [[$dp, $ea]]
Local Const $eb[1][3] = [[$dp, $ds, True]]
Local Const $ec[1][3] = [[$dp, "HKLM" & $7x & "\SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\combofix.exe", False]]
_vv($cf, $dw)
_vv($cf, $dx)
_vv($ch, $dw)
_vv($ch, $dx)
...
Local Const $dw[1][3] = [[$dp, $ds, False]]
Local Const $dx[1][5] = [[$dp, 'file', $cz, $ds, False]]
Local Const $dy[1][5] = [[$dp, 'file', $cz, $dt, False]]
_vv($ce, $dw)
_vv($cf, $dx)
_vv($cf, $dy)
_vv($ch, $dx)
...
Local Const $dw[1][3] = [[$dp, $ds, False]]
Local Const $dx[1][5] = [[$dp, 'file', $cz, $ds, Null]]
Local Const $dy[1][5] = [[$dp, 'file', $cz, $dt, Null]]
_vv($ce, $dw)
_vv($cf, $dx)
_vv($ch, $dx)
_vv($cj, $dy)
...
Local Const $dx[1][5] = [[$dp, 'file', $cz, $ds, False]]
Local Const $dy[1][5] = [[$dp, 'folder', $cz, $dt, False]]
Local Const $dz[1][5] = [[$dp, 'file', $cz, $du, False]]
_vv($ce, $dw)
_vv($cf, $dx)
_vv($cf, $dy)
_vv($ch, $dx)
...
Local Const $dx[1][5] = [[$dp, 'file', $cz, $ds, False]]
Local Const $dy[1][5] = [[$dp, 'file', $cz, $dt, False]]
Local Const $dz[1][5] = [[$dp, 'folder', $cz, $du, True]]
Local Const $e5[1][5] = [[$dp, 'file', $cz, $dv, False]]
_vv($ce, $dw)
_vv($cf, $dx)
_vv($ch, $dx)
_vv($cj, $dy)
...
Dim $cg
Dim $ck
Local Const $cz = "(?i).*VBS autorun worms.*"
Local Const $e4 = Null
...
Local Const $dw[1][3] = [[$dp, $ds, False]]
Local Const $dx[1][5] = [[$dp, 'file', $e4, $ds, False]]
Local Const $dy[1][5] = [[$dp, 'file', $e4, $dt, False]]
Local Const $dz[1][5] = [[$dp, 'folder', $cz, $du, True]]
Local Const $e5[1][2] = [[$dp, $dv]]
Local Const $e7[1][5] = [[$dp, 'file', $e4, $dv, False]]
_vv($ce, $dw)
_vv($ce, $e5)
_vv($cf, $dx)
_vv($cf, $e7)
...
Local Const $dw[1][3] = [[$dp, $ds, False]]
Local Const $dx[1][5] = [[$dp, 'file', $cz, $dt, True]]
Local Const $dy[1][5] = [[$dp, 'file', Null, $du, True]]
Local Const $dz[1][5] = [[$dp, 'file', Null, $dv, True]]
Local Const $e5[1][5] = [[$dp, 'folder', Null, $e9, True]]
_vv($ce, $dw)
_vv($cf, $dx)
_vv($ch, $dx)
_vv($cf, $dy)
...
Local Const $dw[1][3] = [[$dp, $ed, False]]
Local Const $dx[1][4] = [[$dp, "HKLM" & $7x & "\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall", $ds, "DisplayName"]]
Local Const $dy[1][5] = [[$dp, 'folder', Null, $dt, True]]
Local Const $dz[1][5] = [[$dp, 'folder', Null, $du, True]]
Local Const $e5[1][5] = [[$dp, 'file', Null, $dv, False]]
Local Const $e7[1][5] = [[$dp, 'file', Null, $e9, False]]
Local Const $eb[1][5] = [[$dp, 'file', Null, $ea, False]]
Local Const $ec[1][2] = [[$dp, $ee]]
_vv($ce, $dw)
_vv($ce, $ec)
_vv($cm, $dx)
_vv($ci, $dy)
...
Local Const $dw[1][3] = [[$dp, $ds, False]]
Local Const $dx[1][5] = [[$dp, 'file', $cz, $ds, False]]
Local Const $dy[1][3] = [[$dp, $ef, $dt]]
Local Const $dz[1][3] = [[$dp, "HKLM" & $7x & "\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\SEAF", False]]
Local Const $e5[1][5] = [[$dp, 'file', Null, $du, False]]
Local Const $e7[1][5] = [[$dp, 'folder', Null, $ef, True]]
_vv($ce, $dw)
_vv($cf, $dx)
_vv($ch, $dx)
_vv($cs, $dy)
...
Local Const $cz = "(?i)^The RaProducts Team"
...
Local Const $cz = "(?i)^Bleeping Computer"
...
_12l()
Func _12m()
Local Const $cz = "(?i)^Symantec"
Local Const $dp = "Symantec Kovter Removal Tool"
Dim $ce
Dim $cf
Dim $ch
Local Const $ds = "(?i)^FixTool.*\.exe$"
Local Const $dt = "(?i)^FixTool.*\.(log|exe)$"
Local Const $dw[1][3] = [[$dp, $ds, False]]
Local Const $dx[1][5] = [[$dp, 'file', $cz, $dt, False]]
_vv($ce, $dw)
_vv($cf, $dx)
...
_12m()
Func _12n()
Local Const $cz = "(?i)^Alex Dragokas"
Local Const $dp = "Check Browsers LNK"
Dim $ce
Dim $cf
Dim $ch
Local Const $ds = "(?i)^Check(-|_| )Browsers(-|_| )LNK.*\.exe$"
Local Const $dt = "(?i)^Check(-|_| )Browsers(-|_| )LNK.*\.(log|exe)$"
Local Const $dw[1][3] = [[$dp, $ds, False]]
Local Const $dx[1][5] = [[$dp, 'file', Null, $dt, False]]
_vv($ce, $dw)
_vv($cf, $dx)
...
Dim $cf
Dim $ch
Local Const $dp = "Crypt38Decrypter"
Local Const $ds = "(?i)^Crypt38Decrypter.*\.exe$"
Local Const $dt = "(?i)^Crypt38(KeyGen|Decrypter).*\.(exe|zip|txt)$"
Local Const $du = "(?i)^Crypt38(KeyGen|Decrypter)$"
Local Const $dw[1][3] = [[$dp, $ds, False]]
Local Const $dx[1][5] = [[$dp, 'file', Null, $dt, False]]
Local Const $dy[1][5] = [[$dp, 'folder', Null, $du, False]]
_vv($ce, $dw)
...
Dim $cf
Dim $ch
Local Const $dp = "PowerLockyDecrypter"
Local Const $ds = "(?i)^PowerLockyDecrypter.*\.exe$"
Local Const $dt = "(?i)^PowerLockyDecrypter.*\.(exe|zip|txt)$"
Local Const $du = "(?i)^PowerLockyDecrypter$"
Local Const $dw[1][3] = [[$dp, $ds, False]]
Local Const $dx[1][5] = [[$dp, 'file', Null, $dt, False]]
Local Const $dy[1][5] = [[$dp, 'folder', Null, $du, False]]
_vv($ce, $dw)
...
Dim $cf
Dim $ch
Local Const $dp = "InsaneCryptDecrypter"
Local Const $ds = "(?i)^InsaneCryptDecrypter.*\.exe$"
Local Const $dt = "(?i)^InsaneCryptDecrypter.*\.(exe|zip|txt)$"
Local Const $du = "(?i)^InsaneCryptDecrypter$"
Local Const $dw[1][3] = [[$dp, $ds, False]]
Local Const $dx[1][5] = [[$dp, 'file', Null, $dt, False]]
Local Const $dy[1][5] = [[$dp, 'folder', Null, $du, False]]
_vv($ce, $dw)
...
Dim $cf
Dim $ch
Local Const $dp = "BTCWareDecrypter"
Local Const $ds = "(?i)^BTCWareDecrypter.*\.exe$"
Local Const $dt = "(?i)^BTCWareDecrypter.*\.(exe|zip|txt)$"
Local Const $du = "(?i)^BTCWareDecrypter$"
Local Const $dw[1][3] = [[$dp, $ds, False]]
Local Const $dx[1][5] = [[$dp, 'file', Null, $dt, False]]
Local Const $dy[1][5] = [[$dp, 'folder', Null, $du, False]]
_vv($ce, $dw)
...
Dim $cf
Dim $ch
Local Const $dp = "AlphaDecrypter"
Local Const $ds = "(?i)^AlphaDecrypter.*\.exe$"
Local Const $dt = "(?i)^AlphaDecrypter.*\.(exe|zip|txt)$"
Local Const $du = "(?i)^AlphaDecrypter$"
Local Const $dw[1][3] = [[$dp, $ds, False]]
Local Const $dx[1][5] = [[$dp, 'file', Null, $dt, False]]
Local Const $dy[1][5] = [[$dp, 'folder', Null, $du, False]]
_vv($ce, $dw)
...
Dim $cf
Dim $ch
Local Const $dp = "HiddenTearDecrypter"
Local Const $ds = "(?i)^HiddenTearDecrypter.*\.exe$"
Local Const $dt = "(?i)^HiddenTearDecrypter.*\.(exe|zip|txt)$"
Local Const $du = "(?i)^HiddenTearDecrypter.*$"
Local Const $dw[1][3] = [[$dp, $ds, False]]
Local Const $dx[1][5] = [[$dp, 'file', Null, $dt, False]]
Local Const $dy[1][5] = [[$dp, 'folder', Null, $du, False]]
_vv($ce, $dw)
...
Dim $cf
Dim $ch
Local Const $dp = "StupidDecrypter"
Local Const $ds = "(?i)^StupidDecrypter.*\.exe$"
Local Const $dt = "(?i)^StupidDecrypter.*\.(exe|zip|txt)$"
Local Const $du = "(?i)^StupidDecrypter.*$"
Local Const $dw[1][3] = [[$dp, $ds, False]]
Local Const $dx[1][5] = [[$dp, 'file', Null, $dt, False]]
Local Const $dy[1][5] = [[$dp, 'folder', Null, $du, False]]
_vv($ce, $dw)
...
Dim $cf
Dim $ch
Local Const $dp = "GibonDecrypter"
Local Const $ds = "(?i)^GibonDecrypter.*\.exe$"
Local Const $dt = "(?i)^GibonDecrypter.*\.(exe|zip|txt)$"
Local Const $du = "(?i)^GibonDecrypter$"
Local Const $dw[1][3] = [[$dp, $ds, False]]
Local Const $dx[1][5] = [[$dp, 'file', Null, $dt, False]]
Local Const $dy[1][5] = [[$dp, 'folder', Null, $du, False]]
_vv($ce, $dw)
...
Dim $cf
Dim $ch
Local Const $dp = "AuroraDecrypter"
Local Const $ds = "(?i)^AuroraDecrypter.*\.exe$"
Local Const $dt = "(?i)^AuroraDecrypter.*\.(exe|zip|txt)$"
Local Const $du = "(?i)^AuroraDecrypter$"
Local Const $dw[1][3] = [[$dp, $ds, False]]
Local Const $dx[1][5] = [[$dp, 'file', Null, $dt, False]]
Local Const $dy[1][5] = [[$dp, 'folder', Null, $du, False]]
_vv($ce, $dw)
...
Dim $cf
Dim $ch
Local Const $dp = "FilesLockerDecrypter"
Local Const $ds = "(?i)^FilesLockerDecrypter.*\.exe$"
Local Const $dt = "(?i)^FilesLockerDecrypter.*\.(exe|zip|txt)$"
Local Const $du = "(?i)^FilesLockerDecrypter$"
Local Const $dw[1][3] = [[$dp, $ds, False]]
Local Const $dx[1][5] = [[$dp, 'file', Null, $dt, False]]
Local Const $dy[1][5] = [[$dp, 'folder', Null, $du, False]]
_vv($ce, $dw)
...
Dim $cf
Dim $ch
Local Const $dp = "DCryDecrypter"
Local Const $ds = "(?i)^DCryDecrypter.*\.exe$"
Local Const $dt = "(?i)^DCryDecrypter.*\.(exe|zip|txt)$"
Local Const $du = "(?i)^DCryDecrypter$"
Local Const $dw[1][3] = [[$dp, $ds, False]]
Local Const $dx[1][5] = [[$dp, 'file', Null, $dt, False]]
Local Const $dy[1][5] = [[$dp, 'folder', Null, $du, False]]
_vv($ce, $dw)
...
Dim $cf
Dim $ch
Local Const $dp = "StrikedDecrypter"
Local Const $ds = "(?i)^StrikedDecrypter.*\.exe$"
Local Const $dt = "(?i)^StrikedDecrypter.*\.(exe|zip|txt)$"
Local Const $du = "(?i)^StrikedDecrypter$"
Local Const $dw[1][3] = [[$dp, $ds, False]]
Local Const $dx[1][5] = [[$dp, 'file', Null, $dt, False]]
Local Const $dy[1][5] = [[$dp, 'folder', Null, $du, False]]
_vv($ce, $dw)
...
Dim $cf
Dim $ch
Local Const $dp = "Unlock92Decrypter"
Local Const $ds = "(?i)^Unlock92Decrypter.*\.exe$"
Local Const $dt = "(?i)^Unlock92Decrypter.*\.(exe|zip|txt)$"
Local Const $du = "(?i)^Unlock92Decrypter$"
Local Const $dw[1][3] = [[$dp, $ds, False]]
Local Const $dx[1][5] = [[$dp, 'file', Null, $dt, False]]
Local Const $dy[1][5] = [[$dp, 'folder', Null, $du, False]]
_vv($ce, $dw)
...
Dim $cf
Dim $ch
Local Const $dp = "BitStakDecrypter"
Local Const $ds = "(?i)^BitStakDecrypter.*\.exe$"
Local Const $dt = "(?i)^BitStakDecrypter.*\.(exe|zip|txt)$"
Local Const $du = "(?i)^BitStakDecrypter$"
Local Const $dw[1][3] = [[$dp, $ds, False]]
Local Const $dx[1][5] = [[$dp, 'file', Null, $dt, False]]
Local Const $dy[1][5] = [[$dp, 'folder', Null, $du, False]]
_vv($ce, $dw)
...
Dim $cf
Dim $ch
Local Const $dp = "BitKangarooDecrypter"
Local Const $ds = "(?i)^BitKangarooDecrypter.*\.exe$"
Local Const $dt = "(?i)^BitKangarooDecrypter.*\.(exe|zip|txt)$"
Local Const $du = "(?i)^BitKangarooDecrypter$"
Local Const $dw[1][3] = [[$dp, $ds, False]]
Local Const $dx[1][5] = [[$dp, 'file', Null, $dt, False]]
Local Const $dy[1][5] = [[$dp, 'folder', Null, $du, False]]
_vv($ce, $dw)
...
Dim $cf
Dim $ch
Local Const $dp = "MirCopDecrypter"
Local Const $ds = "(?i)^MirCopDecrypter.*\.exe$"
Local Const $dt = "(?i)^MirCopDecrypter.*\.(exe|zip|txt)$"
Local Const $du = "(?i)^MirCopDecrypter$"
Local Const $dw[1][3] = [[$dp, $ds, False]]
Local Const $dx[1][5] = [[$dp, 'file', Null, $dt, False]]
Local Const $dy[1][5] = [[$dp, 'folder', Null, $du, False]]
_vv($ce, $dw)
...
_10l(@DesktopDir, $cf)
_105()
_10m(@DesktopCommonDir, $cg)
_105()
If FileExists(@UserProfileDir & "\Downloads") Then
_10l(@UserProfileDir & "\Downloads", $ch)
_105()
Else
_105()
EndIf
...
_10m(@AppDataDir, $co)
_105()
_10m(@AppDataCommonDir, $cn)
_105()
_10m(@LocalAppDataDir, $cp)
_105()
_10m(@WindowsDir, $cr)
_105()
_10t($ck)
...
_10u($cm)
_105()
_10m(@AppDataCommonDir & "\Microsoft\Windows\Start Menu\Programs", $cq)
_105()
_10w($cu)
_105()
If $bu = True Then
